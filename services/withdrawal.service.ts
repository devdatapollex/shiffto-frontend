...
export async function requestWithdrawal(
  ...
  const { data } = await apiClient.post<{ data: WithdrawalHistoryItem }>('/withdrawals/request', payload);
  return data.data;
}

export async function getMyWithdrawals(): Promise<WithdrawalHistoryItem[]> {
  const { data } = await apiClient.get<{ data: WithdrawalHistoryItem[] }>('/withdrawals/my-withdrawals');
  return data.data;
}

export async function getAllWithdrawals(status?: string): Promise<WithdrawalHistoryItem[]> {
  const params = status ? { status } : {};
  const { data } = await apiClient.get<{ data: WithdrawalHistoryItem[] }>('/withdrawals/admin/all', { params });
  return data.data;
}
