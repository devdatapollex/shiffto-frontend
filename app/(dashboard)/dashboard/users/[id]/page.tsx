  ...
  if (error || !userDetails) {
    return (
      <div className="text-center py-20">
        ...
        <Button asChild className="mt-5 rounded-xl border border-primary/10">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6 lg:col-span-1">
          <div className="p-6 border border-primary/5 rounded-lg bg-card space-y-6">
            ...
            <div className="flex flex-col items-center text-center pb-4 border-b border-primary/5">
              ...
              <Badge className={`border uppercase text-[9px] font-bold mt-2 py-0.5 px-2 ${STATUS_COLORS[profile.status]}`}>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <Label htmlFor="userName" className="text-xs font-semibold">Name</Label>
                <Input
                  id="userName"
                  value={editName}
              ...
              <div>
                <Label htmlFor="userPhone" className="text-xs font-semibold">Phone Number</Label>
                <Input
                  id="userPhone"
                  value={editPhone}
              ...
              <div>
                <Label htmlFor="userStatus" className="text-xs font-semibold">Account Status</Label>
                <Select value={editStatus} onValueChange={(val) => setEditStatus(val)}>
                  <SelectTrigger id="userStatus" className="border-primary/10 mt-1 focus:ring-primary">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Navigation Tabs */}
          <div className="flex border-b border-primary/5 bg-primary/[0.01] p-1 rounded-xl border border-primary/5">
            {[
          ...
          <div className="p-6 border border-primary/5 rounded-lg bg-card min-h-[400px]">
            ...
            {activeTab === 'timeline' && (
              <div className="space-y-6">
                <h3 className="text-sm font-bold border-b border-primary/5 pb-2">Mixed Activity Timeline</h3>
                {timeline.length === 0 ? (
                  <p className="text-xs text-muted-foreground italic py-8 text-center">No recorded activity for this user.</p>
                ) : (
            ...
            {activeTab === 'kyc' && (
              <div className="space-y-6">
                <div className="flex justify-between items-center border-b border-primary/5 pb-2">
                  ...
                  {kyc && (
                    <Badge className={`border uppercase text-[9px] font-bold py-0.5 px-2 ${
                ...
                {!kyc ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-xs text-muted-foreground italic">KYC verification documents have not been submitted by this user.</p>
                  </div>
                ) : (
                  <div className="space-y-6">
                    {/* Metadata fields */}
                    <div className="grid grid-cols-2 gap-4 text-xs">
                      <div className="p-3 bg-muted/30 border border-primary/5 rounded-xl">
                        <span className="text-muted-foreground block text-[10px] font-semibold uppercase">Document Type</span>
                        <strong className="text-foreground mt-0.5 block">{kyc.documentType}</strong>
                      </div>
                      <div className="p-3 bg-muted/30 border border-primary/5 rounded-xl">
                        <span className="text-muted-foreground block text-[10px] font-semibold uppercase">Document Number</span>
                        <strong className="text-foreground mt-0.5 block">{kyc.documentNumber}</strong>
                      </div>
                      <div className="p-3 bg-muted/30 border border-primary/5 rounded-xl">
                        <span className="text-muted-foreground block text-[10px] font-semibold uppercase">Nationality</span>
                        <strong className="text-foreground mt-0.5 block">{kyc.nationality}</strong>
                      </div>
                      <div className="p-3 bg-muted/30 border border-primary/5 rounded-xl">
                        <span className="text-muted-foreground block text-[10px] font-semibold uppercase">Phone Verified</span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                      <div className="space-y-2">
                        <span className="text-xs font-semibold text-muted-foreground">Document Front Side</span>
                        <div className="relative border border-primary/5 rounded-xl overflow-hidden bg-muted aspect-video flex items-center justify-center">
                          <img src={kyc.frontPhotoUrl} alt="Document Front" className="h-full w-full object-contain" />
                          <a href={kyc.frontPhotoUrl} target="_blank" rel="noreferrer" className="absolute right-2 bottom-2 p-1.5 bg-black/60 rounded-lg text-white hover:bg-black/80 transition-colors">
                            <ExternalLink className="h-3.5 w-3.5" />
                          </a>
                        </div>
                      </div>
                      <div className="space-y-2">
                        <span className="text-xs font-semibold text-muted-foreground">Document Back Side</span>
                        <div className="relative border border-primary/5 rounded-xl overflow-hidden bg-muted aspect-video flex items-center justify-center">
                          <img src={kyc.backPhotoUrl} alt="Document Back" className="h-full w-full object-contain" />
                          <a href={kyc.backPhotoUrl} target="_blank" rel="noreferrer" className="absolute right-2 bottom-2 p-1.5 bg-black/60 rounded-lg text-white hover:bg-black/80 transition-colors">
                            <ExternalLink className="h-3.5 w-3.5" />
                          </a>
                        </div>
                      </div>
            ...
            {activeTab === 'transactions' && (
              <div className="space-y-4">
                <h3 className="text-sm font-bold border-b border-primary/5 pb-2">Financial Transactions Log</h3>
                {transactions.length === 0 ? (
                  <p className="text-xs text-muted-foreground italic py-8 text-center">No transaction records found for this user.</p>
                ) : (
                    ...
                    <table className="w-full text-xs text-left border-collapse">
                      ...
                      <tbody>
                        {transactions.map((tx) => {
                          ...
                          return (
                            <tr key={tx.id} className="border-b border-primary/5 hover:bg-primary/[0.01]">
                              <td className="py-2.5 font-mono text-[10px]">{tx.transactionId}</td>
                              <td className="py-2.5 font-semibold truncate max-w-[150px]">{tx.shipment.itemName}</td>
                              <td className="py-2.5 font-bold text-foreground">${tx.grossAmount.toFixed(2)}</td>
                              <td className="py-2.5">
                                <Badge variant="outline" className={`text-[9px] uppercase ${
                                  isSender ? 'text-blue-600 border-blue-100 bg-blue-50/10' : 'text-emerald-600 border-emerald-100 bg-emerald-50/10'
                                }`}>
            ...
            {activeTab === 'tickets' && (
              <div className="space-y-4">
                <h3 className="text-sm font-bold border-b border-primary/5 pb-2">Support Tickets</h3>
                {tickets.length === 0 ? (
                  <p className="text-xs text-muted-foreground italic py-8 text-center">No support tickets created by this user.</p>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs text-left border-collapse">
                      <thead>
                      ...
                      <tbody>
                        {tickets.map((t) => (
                          <tr key={t.id} className="border-b border-primary/5 hover:bg-primary/[0.01]">
                            <td className="py-2.5 font-bold text-primary font-mono">{t.ticketId}</td>
                            <td className="py-2.5 uppercase text-[10px] font-semibold">{t.category}</td>
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between gap-4 p-4 border border-primary/5 rounded-lg bg-primary/[0.01] items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-[10px] text-muted-foreground uppercase font-semibold">Account Trust Score</span>
                      <strong className="text-xl font-extrabold text-foreground block">{profile.trustScore} / 100</strong>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-center">
                    <div>
                      ...
                      <strong className="text-sm font-bold text-foreground block">{reviews.received.length} Ratings</strong>
                    </div>
                    <div className="w-px h-8 bg-primary/10" />
                    <div>
                      <span className="text-[10px] text-muted-foreground block">Given</span>
                      <strong className="text-sm font-bold text-foreground block">{reviews.given.length} Ratings</strong>
                    </div>
                  </div>
                </div>
                ...
                {/* Reviews List */}
                <div className="space-y-4">
                  <h3 className="text-sm font-bold border-b border-primary/5 pb-2">Reviews Received ({reviews.received.length})</h3>
                  {reviews.received.length === 0 ? (
                    <p className="text-xs text-muted-foreground italic py-4 text-center">No reviews received yet.</p>
                  ) : (
                    <div className="space-y-3">
                      {reviews.received.map((rev) => (
                        <div key={rev.id} className="p-3 border border-primary/5 rounded-xl bg-card/50 space-y-1.5">
                          ...
                          {rev.comment && <p className="text-xs text-muted-foreground leading-relaxed pl-8 italic">"{rev.comment}"</p>}
