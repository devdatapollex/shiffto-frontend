        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-primary/10 via-card to-card p-6 rounded-2xl border border-primary/10 shadow-xs">
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
              Monitor system activity, manage pending verifications, track transaction volume, and respond to support tickets.
            </p>
          </div>
          ...
          <Card className="border-amber-500/20 bg-gradient-to-br from-amber-500/5 to-transparent shadow-xs">
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs text-muted-foreground font-medium">Pending KYC Verifications</div>
                  <div className="text-xl font-bold text-foreground">
                    {isLoading ? '...' : stats?.pendingKycCount || 0}
                  </div>
                </div>
              </div>
              <Button asChild size="xs" variant="outline" className="text-amber-700 border-amber-500/30 hover:bg-amber-500/10 font-semibold text-xs gap-1">
          ...
          <Card className="border-blue-500/20 bg-gradient-to-br from-blue-500/5 to-transparent shadow-xs">
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs text-muted-foreground font-medium">Open Support Tickets</div>
                  <div className="text-xl font-bold text-foreground">
                    {isLoading ? '...' : stats?.openTicketsCount || 0}
                  </div>
                </div>
              </div>
              <Button asChild size="xs" variant="outline" className="text-blue-700 border-blue-500/30 hover:bg-blue-500/10 font-semibold text-xs gap-1">
          ...
          <Card className="border-emerald-500/20 bg-gradient-to-br from-emerald-500/5 to-transparent shadow-xs">
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs text-muted-foreground font-medium">Pending Withdrawals</div>
                  <div className="text-xl font-bold text-foreground">
                    {isLoading ? '...' : stats?.pendingWithdrawalsCount || 0}
                  </div>
                </div>
              </div>
              <Button asChild size="xs" variant="outline" className="text-emerald-700 border-emerald-500/30 hover:bg-emerald-500/10 font-semibold text-xs gap-1">
            ...
            <Card className="border-primary/10 hover:border-primary/30 transition-all shadow-xs">
              <CardHeader className="pb-2">
                <CardDescription className="flex items-center justify-between text-xs font-semibold">
                  ...
                  <Badge variant="outline" className="text-[9px] font-bold border-emerald-500/20 text-emerald-700 bg-emerald-50 dark:bg-emerald-950/20">
            ...
            <Card className="border-primary/10 hover:border-primary/30 transition-all shadow-xs">
              <CardHeader className="pb-2">
                <CardDescription className="flex items-center justify-between text-xs font-semibold">
                  ...
                  <Badge variant="outline" className="text-[9px] font-bold border-blue-500/20 text-blue-700 bg-blue-50 dark:bg-blue-950/20">
            ...
            <Card className="border-primary/10 hover:border-primary/30 transition-all shadow-xs">
              ...
              <CardContent>
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <Badge variant="secondary" className="text-[10px] font-semibold bg-emerald-500/10 text-emerald-700 border-none">
        ...
        <Card className="border-primary/10 shadow-xs">
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <CardTitle className="text-base font-bold">Platform Activity & Revenue Trend</CardTitle>
                    ...
                    {recentKyc.map((kyc) => (
                      <TableRow key={kyc.id} className="text-xs">
                        <TableCell className="font-medium">
                          <div className="font-semibold text-foreground">{kyc.user?.name || 'User'}</div>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button
                              ...
                              onClick={() => kycMutation.mutate({ kycId: kyc.id, status: 'APPROVED' })}
                              title="Approve KYC"
                            >
                            ...
                            <Button
                              ...
                              disabled={kycMutation.isPending}
                              onClick={() => kycMutation.mutate({ kycId: kyc.id, status: 'REJECTED' })}
                              title="Reject KYC"
                            >
